...
For implementing language features and creating plans for it, always follow the documentation present on @docs/, unless asked otherwise. Inside the docs folder, there will be another set of folders, one for each version. You shall only access the latest one when you need to reference it. The versions are not sorted alphabetically, in a sense that v0.12 is more recent than v0.2, so you need to follow v0.12.
...
Whenever there has been a substancial change to the language, update the README.md file to reflect it. The docs in the docs directory should be the 'idealized' version of the language, where the README contains what is really implemented.

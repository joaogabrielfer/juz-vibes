...
`:=` remains accepted in v0.6 examples as the inferred binding shorthand. The future style
...
- Under `@notation`, `@macro`, and bang-callable `@comptime`, call-shape attributes
  (`@position`, `@bind`, `@assoc`, `@precedence`, `@adjacent`) go on the line above the
  `let` declaration and are written on a single line.
...
in depth in [[26-attributes-metadata]].
...
Under `@notation`, `@macro`, and bang-callable `@comptime`, additional declaration-level
attributes configure call shape:
...
File embedding and similar compile-time data loading are provided by bang-callable
`@comptime` elements or plugins, for example:

...
Package and module paths use `::`. Dot access is reserved for structural subelements and
variant/member-like value shapes, not package traversal.

Local modules use `<package_name>::<module_name>`. `std` is the default standard package
root and is addressable without an explicit package import. This is separate from the
prelude: disabling `std::prelude` removes unqualified prelude names but does not remove
explicit `std::...` paths.

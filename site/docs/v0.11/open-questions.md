...
This file collects unresolved design issues for v0.11. Items marked as carried from v0.3
...
- inferred generic constraints can produce confusing diagnostics if arithmetic traits,
  numeric literals, and notation dispatch are not already stable.
...
subtyping should be considered only if concrete implementation pressure appears. Inferred
generic constraints should stay on the backburner until the core trait surface is stable.
...
```rust
ini deploy: (target: string) -> Fallible<void> = { ... }
```
...
3. Does a non-`void` return print, affect exit code, or feed another `ini`?

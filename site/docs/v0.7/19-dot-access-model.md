...
- `foo.computed` - access a zero-argument computed subelement from an implementation,
  with `self` implicitly bound to `foo`

Attached subelements declared with `sum ... with {}` are stored subelements and use the
same direct access model (`err.meta`, `err.meta.notes`, ...).
...
when their types are known from the trait. Computed subelements that are not declared by
a trait may specify their type explicitly.

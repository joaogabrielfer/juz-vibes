...
let read_numbers :: Path -> Fallible<unit>;
...
@handles(Yield<T>)
let (collect): <T, C = Arr> (unit -> unit) -> C<T>
```
...
Bare access to a stream cursor or helper must not advance a stream implicitly through
zero-argument access rules.
...
- pure zero-argument elements are demand-driven and may reuse a stored result on bare
  access
- effectful zero-argument elements are demand-driven and uncached
...
let read_config_text :: Path -> Fallible<string>
let read_config_text = path -> {
...
- advanced refinement solving beyond local proof rules

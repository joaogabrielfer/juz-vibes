...
`::` is namespace and associated-item lookup. If a module, type, or trait with the same
name is visible in the same scope and the lookup is ambiguous, the compiler must reject
the reference until the user imports one of the names with an alias or uses a more
specific path.

...
- Added the formal core pattern model covering pattern alternatives, alias patterns,
  pinned value patterns, product and variant patterns, sequence rest patterns, regex
  patterns, and redundancy/exhaustiveness rules.
- Added pattern-matching future work notes for `do` view/extractor groups, pattern
  synonyms, map patterns, literal macro details, sequence rest representation, and
  GADT-like narrowing.

### Changed

- Clarified generic union overlap rules for pattern-matched functions.
- Clarified that repeated binders in a single pattern are invalid and should be expressed
  with guards.
...
- Added the numeric tower baseline: `int` as `i64`, `float` as `f64`, pointer-sized
  `usize` and `isize`, exact compile-time numeric literals, and `Byte` as a standard
  alias for `u8`.
- Added numeric capability traits for arithmetic, fixed-width bounds, signedness,
  literal conversion, and indexing-related conversion.

### Changed

- Narrowed the remaining numeric open questions to overflow behavior, cast APIs, checked
...
- Added `v0.12` as a new minor documentation version copied from `v0.11`.
- Added lightweight refinement types with `where` predicates and local proof rules.
- Added fixed-size array type expressions `T[N]` and `T[N: I]`.
- Added bounded dynamic array syntax `Arr<T>[I]`.
- Added indexing-domain traits for finite and sequential index types.
- Added explicit safe-indexing rules where `[]` must be total or compiler-proven safe.
- Added `unit` as the one-value type and `never` as the zero-value type.

### Changed

- Changed the documented zero-argument callable type from `void -> T` to `unit -> T`.
- Changed standard-library examples and signatures from `void`/`Never` to `unit`/`never`
  where they describe current semantics.
- Changed `std::collections` from a placeholder page to a draft specification of dynamic
  arrays, bounded arrays, and partial indexing APIs.
- Changed function and macro examples to follow the current one-arrow rule for typed
  declarations.
- Changed the open questions around refinements to focus on proof depth and future
  typestate work instead of initial syntax.

...
```txt
let [mut] [attrs] name [: type] = body
let [mut] [attrs] name :: type;
let [mut] [attrs] name = arg_patterns -> body
```
...
An inline function annotation may bind parameter names inside the type. Those names are
then available inside a block body. A lambda body that binds arguments with `->` must use
an inferred binding or a standalone `::` signature instead.
...
A declaration cannot use two argument-binding arrows at once. It either binds arguments
in the type annotation, or it binds arguments in the lambda body.
...
`unit -> T` declares a callable zero-argument element returning `T`. A plain `T`
...
while `let now: unit -> Time = ...` is invoked as `now()` and never receives bare-access
caching.
...
required parameters after defaults when all following arguments are named, but the v0.12

...
let html!: (markup: TokenStream) -> TokenStream =  { ... }
...
```rust
@comptime
let build_lookup: (ops: Arr<string>) -> Arr<(string, int)> = {
...
```rust
let read_into_string: (p: Path) -> string = { ... }
...
`@run`, `@insert`, and `@if` remain statement-level compile-time directives. Compile-time

...
the holes. Missing required arguments without `_` are errors and do not create implicit
...
Omitted defaulted parameters use their defaults. An explicit `_` for a defaulted
parameter creates a partial application slot instead of using the default.
...
let (echo): <T: Display> (x: T) -> unit = { print Display::show(x) }
```

...
| Value only | `let name = value` | Value definition, type declared previously or inferred from context |
| Inferred binding shorthand | `let name := value` | Value definition, type inferred |
| Annotated assignment | `let name: type = value` | Inline type plus value. For function types, parameter bindings belong either in the type or in the body, not both |

`let name :: type` declarations are valid only at module scope. Inside `{}` blocks,
values must be defined in dependency order.

When `type` is a function type, the declaration must use only one argument-binding arrow.
...
```rust
@macro
let json!: (stream: TokenStream) -> TokenStream = { ... }

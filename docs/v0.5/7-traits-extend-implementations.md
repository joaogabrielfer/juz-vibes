...
The compiler determines at use-time whether a trait field is a method field, because its
type is a function, or a data field, because its type is a value.
...
field defines the runtime data layout. Other fields satisfy trait requirements or define
computed properties.
...
optional when the trait already determines the field type.
...
When an `extends` block has no `self` field, `self` is implicitly the extended type.
...
Adding a `self` layout when extending an existing type is a type error.

### Field Definition Rules

Inside `extends`, fields that implement trait requirements omit type annotations when the
trait already declares their type.

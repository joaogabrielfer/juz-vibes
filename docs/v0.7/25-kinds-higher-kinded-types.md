...
let k_fall = @run kind_of(Fallible) // Type -> Type
let k_either = @run kind_of(Either) // Type -> Type -> Type
```
...
```rust
let process: def(User |> Payload |> Fallible |> Async) = ...
```
...
let transform_user:
    (Async<Fallible<Payload<User>>>) ->
    Async<Fallible<Payload<UserView>>> =
    stream -> {
